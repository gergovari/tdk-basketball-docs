...
% Szerző [Author]
\def\authorFamilyName{Vári}
\def\authorGivenName{Gergő}
\def\neptun{MQHJ0H}

% Konzulens 1 [Consulent 1]
\def\consulentATitle{}
\def\consulentAFamilyName{Weisz-Tiba}
\def\consulentAGivenName{Klára Julianna}
\def\consulentARank{testnevelő tanár}

% Konzulens 2 [Consulent 2], ha nincs hagyd üresen
\def\consulentBTitle{}
\def\consulentBFamilyName{Ágoston}
\def\consulentBGivenName{Dorottya}
\def\consulentBRank{PhD hallgató}
...
\def\thesisTitle{Kosárlabda büntetődobás biomechanikai elemzése videó alapján}

% Kulcsszavak (a PDF-be) [Keywords (to PDF)]
\def\keywords{kosárlabda, biomechanika, pózbecslés, MediaPipe, kinematika}

% Tanszék [Department]
%TODO Válassz az alábbiak közül
% \bmeatt    \bmeara  \bmeenergia  \bmeepget  \bmegtharom 
% \bmemanuf  \bmehds  \bmemogi     \bmemm     \bmept \bmesport
\def\department{\bmesport}
...
\include{chapters/introduction}          % Bevezetés és Szakirodalmi áttekintés
\include{chapters/materials_and_methods} % Anyagok és módszerek
\include{chapters/results}               % Eredmények
\include{chapters/summary}               % Diszkusszió és Összefoglalás
